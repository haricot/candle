...
fn render_native(
    ...
    // Embed the actual policy identity. The caller's environment is NEVER
    // used as the runtime comparison value.
    let device_gpu_uuid = if required(&policy.fields, "target.scope")? == "device" {
        Some(required(&policy.fields, "target.gpu_uuid")?)
    } else {
        None
    };
    Ok(format!(
...
pub const TARGET_GPU_UUID: Option<&str> = {device_gpu_uuid:?};
