...
fn required<'a>(m: &'a BTreeMap<String, String>, k: &str) -> AResult<&'a str> {
    Ok(m.get(k)
        .ok_or_else(|| format!("missing V2 field {k}"))?
        .as_str())
}
fn n(m: &BTreeMap<String, String>, k: &str) -> AResult<usize> {
    Ok(required(m, k)?.parse()?)
}
fn pos(m: &BTreeMap<String, String>, k: &str) -> AResult<usize> {
    let x = n(m, k)?;
    if x == 0 {
        return Err(format!("zero {k}").into());
    }
    Ok(x)
}
fn dims(m: &BTreeMap<String, String>, k: &str, rank: usize) -> AResult<Vec<usize>> {
    let x = required(m, k)?
        .split('x')
        .map(str::parse)
        .collect::<Result<Vec<usize>, _>>()?;
    if x.len() != rank || x.contains(&0) {
        return Err(format!("bad {k}").into());
    }
    Ok(x)
}
fn kv(s: &str) -> AResult<BTreeMap<String, String>> {
    let mut m = BTreeMap::new();
    for t in s.split(',') {
        let (k, v) = t.split_once('=').ok_or("malformed signature")?;
        if k.is_empty() || v.is_empty() || m.insert(k.into(), v.into()).is_some() {
            return Err("duplicate/empty signature field".into());
        }
    }
    Ok(m)
}
fn parse(text: &str) -> AResult<Policy> {
    ...
    for line in lines {
        ...
        if line.starts_with("decision|") {
            ...
            if p.len() != 8 {
                return Err("decision requires eight fields".into());
            }
            let implementation_id = p[5].strip_prefix("impl=").ok_or("missing impl")?;
            let evidence_ref = p[6].strip_prefix("evidence=").ok_or("missing evidence")?;
            let min = p[7]
                .strip_prefix("min_integrated_speedup_x=")
                .ok_or("missing speedup")?;
            let min_speedup = if min == "none" {
                None
            } else {
                Some(min.parse::<f64>()?)
            };
...
fn common(
    ...
    if s.len() != keys.len() || keys.iter().any(|k| !s.contains_key(*k)) {
        return Err("signature fields differ".into());
    }
    ...
    let st = pos(s, "stride")?;
    let pad = n(s, "padding")?;
    let dil = pos(s, "dilation")?;
    if ci % g != 0 || co % g != 0 {
        return Err("channels/groups mismatch".into());
    }
    let rank = if dim == 1 { 1 } else { 2 };
    let wrank = if dim == 1 { 3 } else { 4 };
    Ok((
        op,
        dim,
        batch,
...
fn validate_evidence_hash(d: &Decision) -> AResult<()> {
    if d.evidence_ref.len() != 64 || !d.evidence_ref.bytes().all(|b| b.is_ascii_hexdigit()) {
        return Err("invalid evidence hash".into());
    }
    Ok(())
}
fn validate_thresholded_proven(d: &Decision) -> AResult<()> {
    validate_evidence_hash(d)?;
    if d.min_speedup.is_none_or(|x| !x.is_finite() || x < 1.1) {
        return Err("invalid thresholded proven metadata".into());
    }
    Ok(())
}
fn specialized_expected(d: &Decision) -> AResult<&'static str> {
    let (op, dim, b, ci, co, sp, w, g, k, st, pad, dil) = common(d)?;
    if b != 1 {
        return Err("specialized batch".into());
    }
    let opad = required(&d.signature, "output_padding")?;
    let id = match (
        op,
        dim,
        ci,
        ...
        ("conv_transpose1d", 1, 128, 128, [32], 2, 3, 2, 1, "1", 1, [128, 64, 3]) => {
            "ct1d-s32-g2-u1-b256"
        }
        ("conv_transpose1d", 1, 128, 128, [32], 4, 3, 2, 1, "1", 1, [128, 32, 3]) => {
            "ct1d-s32-g4-u1-b256"
        }
        ("conv_transpose1d", 1, 128, 128, [32], 8, 3, 2, 1, "1", 1, [128, 16, 3]) => {
            "ct1d-s32-g8-u1-b256"
        }
        ("conv_transpose1d", 1, 128, 128, [32], 16, 3, 2, 1, "1", 1, [128, 8, 3]) => {
            "ct1d-s32-g16-u1-b256"
        }
        ("conv_transpose2d", 2, 128, 128, [32, 32], 16, 3, 2, 1, "1", 1, [128, 8, 3, 3]) => {
            "ct2d-s32-g16-u4-b128"
        }
        ("conv_transpose2d", 2, 128, 128, [32, 32], 32, 3, 2, 1, "1", 1, [128, 4, 3, 3]) => {
            "ct2d-s32-g32-u4-b64"
        }
        ("conv1d", 1, 64, 64, [128], 8, 3, 1, 1, "none", 1, [64, 8, 3]) => "gc1d-l128-g8-u1-b256",
        _ => return Err("specialized geometry differs from frozen table".into()),
    };
    Ok(id)
}
    ...
    match d.implementation_id.as_str() {
        "candle.grouped-transpose.raw.v1" => {
            ...
            if d.id != "ct1d-f32-b1-c64-l128-g2-k3-s2-p1-op1-d1-raw"
                ...
                || st != 2
                || pad != 1
                || required(&d.signature, "output_padding")? != "1"
                || dil != 1
                || d.evidence_ref
                    != "3844cacd61a400dc1979c554e5bce787a11693c265e12b2698dae6a1e855fc47"
            {
                return Err("historical CT1D changed".into());
            }
        }
        "candle.depthwise-conv2d-5x5.raw.v1" => {
            ...
            if op != "conv2d"
                ...
                || dil != 1
                || w != [co, 1, 5, 5]
                || required(&d.signature, "output_padding")? != "none"
                || d.evidence_ref
                    != "84f3dc50433e225b1f63c92a08355b7b04f0afeec49694e5e8d5e040cf092a9a"
            {
                return Err("DW5x5 promoted metadata mismatch".into());
            }
            let ok = matches!(
                (ci, sp.as_slice()),
            ...
            if !ok {
                return Err("DW5x5 exact point unknown".into());
            }
        }
        implementation_id if implementation_id.starts_with("candle.sm61-exact-grouped.") => {
            let cand = specialized_expected(d)?;
            validate_evidence_hash(d)?;
            if d.evidence_ref != SM61_PROMOTION_EVIDENCE
                || d.min_speedup.is_some()
                || d.implementation_id != format!("candle.sm61-exact-grouped.{cand}")
            {
...
fn validate(p: &Policy, build_sm: u32) -> AResult<()> {
    ...
    if required(&p.fields, "target.vendor")? != "nvidia"
        ...
        || required(&p.fields, "target.scope")? != "device"
        || required(&p.fields, "target.sm")? != "61"
        || !policy_uuid.starts_with("GPU-")
        || policy_uuid.len() < 8
        || build_sm != 61
    {
    ...
    let build_uuid =
        env::var("CANDLE_ASD_TARGET_GPU_UUID").map_err(|_| "missing CANDLE_ASD_TARGET_GPU_UUID")?;
    if build_uuid != policy_uuid {
        return Err("build target UUID mismatch".into());
    }
    if p.decisions.len() != 12 {
        return Err("Stage2F requires 12 decisions".into());
    }
    let mut ids = BTreeSet::new();
    let mut sigs = BTreeSet::new();
    let mut states = BTreeMap::<String, usize>::new();
    for d in &p.decisions {
        if !ids.insert(d.id.clone()) || !sigs.insert(format!("{:?}", d.signature)) {
            return Err("duplicate id/signature".into());
        }
        validate_decision(d)?;
        *states.entry(d.state.clone()).or_default() += 1
    }
    if states.get("promoted") != Some(&12) || states.len() != 1 {
        return Err("Stage2F requires 12 promoted decisions".into());
    }
    Ok(())
}
fn rs(s: &str) -> String {
...
fn op_variant(op: &str) -> AResult<&'static str> {
    Ok(match op {
        "conv1d" => "ExactOperation::Conv1d",
        "conv2d" => "ExactOperation::Conv2d",
        "conv_transpose1d" => "ExactOperation::ConvTranspose1d",
        "conv_transpose2d" => "ExactOperation::ConvTranspose2d",
        _ => return Err("unknown op".into()),
    })
}
fn emit(d: &Decision) -> AResult<String> {
    let s = &d.signature;
    let dim = pos(s, "dim")?;
    let sp = dims(s, "spatial", if dim == 1 { 1 } else { 2 })?;
    let w = dims(s, "weight_shape", if dim == 1 { 3 } else { 4 })?;
    let get = |v: &Vec<usize>, i| v.get(i).copied().unwrap_or(0);
    let min = match d.min_speedup {
        Some(x) => format!("Some({x:.8})"),
        None => "None".into(),
    };
    let opad = if required(s, "output_padding")? == "none" {
        0
    } else {
        n(s, "output_padding")?
    };
...
fn render(p: &Policy, validation: bool) -> AResult<String> {
    ...
    let out = t
        ...
        .replace("__DECISIONS__", &rows);
    if out.contains("__DECISIONS__") {
        return Err("unexpanded template".into());
    }
    Ok(out)
}
