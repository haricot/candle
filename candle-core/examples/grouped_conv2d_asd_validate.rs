...
const DEFAULT_WARMUP: usize = 256;
...
impl CustomOp2 for RawExactDw5x5 {
    ...
    fn cpu_fwd(
        ...
        candle_core::bail!("exact DW5x5 ASD validation is CUDA-only")
    }
...
fn main() -> Result<()> {
    ...
    if !candle_kernels::asd_exact_conv2d::VALIDATION_BUILD {
        candle_core::bail!(
            "grouped_conv2d_asd_validate requires an ASD V2 candidate build with CANDLE_ASD_VALIDATION=1"
        )
    }
    ...
    for case in CASES {
        ...
        let current_a = measure(
            Mode::Current, case, &x, &kernel, &device, warmup, iters, inner,
        )?;
        let asd_b = measure(
            Mode::AsdCandidate, case, &x, &kernel, &device, warmup, iters, inner,
        )?;
    ...
    println!(
        "\nMULTI_POINT_GATE cases={} domain_miss={} production_activation=false pass={}",
        CASES.len(), domain_pass, all_pass
    );
    println!("STATUS={}", if all_pass { "PASS" } else { "HOLD" });
    if !all_pass {
        candle_core::bail!("ASD V2 DW5x5 integrated candidate validation failed")
    }
    Ok(())
}
