...
fn launch1d_t<T: DeviceRepr + WithDType + ValidAsZeroBits>(
    ...
    barg!(builder, l_out, p.stride, p.padding, p.output_padding, p.dilation, p.groups);
...
pub(super) fn launch1d(
    ...
    let slice = match (&input.slice, &kernel.slice) {
        ...
        (S::BF16(x), S::BF16(k)) => S::BF16(launch1d_t::<half::bf16>(x, input_l, k, kernel_l, p, &dev)?),
        (S::F16(x), S::F16(k)) => S::F16(launch1d_t::<half::f16>(x, input_l, k, kernel_l, p, &dev)?),
...
pub(super) fn launch2d(
    ...
    let slice = match (&input.slice, &kernel.slice) {
        ...
        (S::BF16(x), S::BF16(k)) => S::BF16(launch2d_t::<half::bf16>(x, input_l, k, kernel_l, p, &dev)?),
        (S::F16(x), S::F16(k)) => S::F16(launch2d_t::<half::f16>(x, input_l, k, kernel_l, p, &dev)?),
