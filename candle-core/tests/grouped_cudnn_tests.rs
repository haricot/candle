...
fn grouped_cudnn_matches_legacy_decomposition() -> Result<()> {
    ...
    // Make a cuDNN failure visible instead of silently accepting the legacy fallback. This keeps
    // the test specific to the native grouped path rather than merely testing grouped conv2d.
...
// The native cuDNN grouped path is deliberately forward-only for now. When either operand tracks
// gradients, Candle must retain the existing chunk -> per-group conv -> cat graph until `groups`
// becomes part of Op::Conv2D/backprop itself.
#[test]
