...
impl Qwen3_5GatedDeltaNet {
    ...
    fn recurrent_gated_delta_rule_step(
        ...
        let query = l2_norm(query)?.transpose(1, 2)?;
        let key = l2_norm(key)?.transpose(1, 2)?;
...
impl Qwen3_5TextRotaryEmbedding {
    pub fn new(dtype: DType, cfg: &Config, dev: &Device) -> Result<Self> {
        ...
        let dim = partial_dim.unwrap_or_else(|| cfg.head_dim());
...
#[derive(Debug, Clone)]
pub(crate) struct Qwen3_5DecoderLayer {
...
pub struct Model {
    ...
    pub(crate) layers: Vec<Qwen3_5DecoderLayer>,
    pub(crate) norm: Qwen3_5RmsNorm,
    device: Device,
    dtype: DType,
}
...
impl Model {
    ...
    pub fn forward_all(
        ...
        let mut hidden_states = if return_hidden {
            Some(vec![xs.clone()])
        } else {
            None
        };
