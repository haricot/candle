...
fn main() -> anyhow::Result<()> {
    ...
    let draft_config: candle_transformers::models::qwen3_5::Config =
        serde_json::from_reader(std::fs::File::open(draft_config_filename)?)?;
    let dflash_config_filename = draft_repo.get("dflash_config.json")?;
    let dflash_config: DFlashConfig =
        serde_json::from_reader(std::fs::File::open(dflash_config_filename)?)?;
    let draft_filenames = vec![draft_repo.get("model.safetensors")?];
    let draft_vb =
        unsafe { VarBuilder::from_mmaped_safetensors(&draft_filenames, dtype, &device)? };
